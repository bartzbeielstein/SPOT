...
spotControl <- function(dimension){
    list(
        ...
        logUncertainties = F, ## Shall uncertainties be logged?
        saveAllModels = F ## return all models instead of just the last one
    )
}
...
spotLoop <- function(x,y,fun,lower,upper,control,parallelCall = F,...){
    ...
    while(count < control$funEvals){ 
        ...
        if(control$saveAllModels){
            allModels <- c(allModels, list(modelFit))
        }
    ...
    list(xbest=x[indexBest,,drop=FALSE],ybest=y[indexBest,,drop=FALSE],x=x, y=y, 
            count=count,msg="budget exhausted",modelFit=modelFit, "loggedUncertainties"= loggedUncertainties)
}	